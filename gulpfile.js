...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/aurelia-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/aurelia-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../aurelia-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/aurelia-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../aurelia-docs/` + changedFileName, `../../../gitlabRepo/aurelia-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/aurelia-docs/')) {
                            shelljs.cp('-rf', `../aurelia-docs/` + changedFileName, `../../../gitlabRepo/aurelia-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/aurelia-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/aurelia-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/aurelia-docs`);
